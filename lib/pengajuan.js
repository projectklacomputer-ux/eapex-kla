...
async function ambil(pengajuanId) {
  ...
  p.realisasi = p.realisasi_json ? bacaJson(p.realisasi_json, null) : null;
  // Rincian per baris (Transport, Hotel, Uang Makan, dst) -- lihat lib/alur.js
  // ajukanRealisasi. p.realisasi.nominal adalah penjumlahannya, bukan diinput manual.
  p.realisasi_items = await db.all(
    'SELECT * FROM pengajuan_realisasi_item WHERE pengajuan_id = ? ORDER BY urut', [pengajuanId]);
  if (p.realisasi_verif_id) {
    const verif = await db.get('SELECT nama FROM pengguna WHERE id = ?', [p.realisasi_verif_id]);
    p.realisasi_verif_nama = verif ? verif.nama : null;
  }

  // Rantai approval ULANG untuk realisasi (lihat lib/alur.js ajukanRealisasi).
  // Kosong untuk dokumen yang tidak memakai alur uang muka, atau yang belum
  // pernah mengajukan realisasi.
  p.persetujuan_realisasi = await db.all(
    'SELECT * FROM persetujuan_realisasi WHERE pengajuan_id = ? ORDER BY urut', [pengajuanId]);
  if (p.persetujuan_realisasi.length) {
    const kandidatRealisasi = await db.all(
      `SELECT kd.persetujuan_realisasi_id, u.id, u.nama, u.jabatan
       FROM persetujuan_realisasi_kandidat kd JOIN pengguna u ON u.id = kd.pengguna_id
       WHERE kd.pengajuan_id = ? ORDER BY u.nama`, [pengajuanId]);
    for (const s of p.persetujuan_realisasi) {
      s.kandidat = kandidatRealisasi.filter(k => k.persetujuan_realisasi_id === s.id);
    }
  }
  return p;
}
...
// Sama seperti langkahAktif/bolehMemutuskan, tapi untuk rantai approval ULANG
// realisasi (lib/alur.js ajukanRealisasi/putuskanRealisasi) — tabel terpisah,
// tidak terikat ke p.langkah_kini milik rantai uang muka yang asli.
function langkahAktifRealisasi(p) {
  return (p.persetujuan_realisasi || []).find(s => s.status === 'menunggu') || null;
}

function bolehMemutuskanRealisasi(p, pengguna) {
  if (p.status_realisasi !== 'diajukan') return false;
  const s = langkahAktifRealisasi(p);
  if (!s) return false;
  return (s.kandidat || []).some(k => k.id === pengguna.id);
}
...
function bolehMelihat(p, pengguna) {
  ...
  // siapa pun yang pernah/akan jadi penyetuju dokumen ini — termasuk di rantai
  // approval ulang realisasi, yang kandidatnya bisa saja berbeda dari rantai asli
  // (mis. Area Manager berganti di antara dua waktu itu)
  const semuaKandidat = (p.persetujuan || []).concat(p.persetujuan_realisasi || []).flatMap(s => s.kandidat || []);
  if (semuaKandidat.some(k => k.id === pengguna.id)) return true;
  if ((p.persetujuan || []).concat(p.persetujuan_realisasi || []).some(s => s.aktor_id === pengguna.id)) return true;
...
// Kotak masuk approval ULANG realisasi: dokumen yang MENUNGGU keputusan
// pengguna ini pada rantai pertanggungjawaban uang muka (persetujuan_realisasi
// — tabel TERPISAH dari persetujuan, lihat lib/alur.js ajukanRealisasi).
//
// SEMUA tahap yang belum diputuskan tersimpan berstatus 'menunggu' sejak awal
// (persis seperti tabel persetujuan yang asli) — bukan cuma tahap yang sedang
// aktif. Tanpa syarat urut = MIN(urut) di bawah, tahap 3 (mis. Accounting) akan
// ikut nyasar ke kotak masuknya padahal belum gilirannya sama sekali.
async function kotakMasukRealisasi(penggunaId) {
  return db.all(
    `SELECT ${PILIH_RINGKAS}, p.realisasi_json, p.realisasi_diajukan,
            s.urut AS langkah_urut, s.label AS langkah_label, s.peran AS langkah_peran
     ${DARI}
     JOIN persetujuan_realisasi s ON s.pengajuan_id = p.id AND s.status = 'menunggu'
       AND s.urut = (SELECT MIN(urut) FROM persetujuan_realisasi WHERE pengajuan_id = p.id AND status = 'menunggu')
     JOIN persetujuan_realisasi_kandidat kd ON kd.persetujuan_realisasi_id = s.id AND kd.pengguna_id = ?
     WHERE p.status_realisasi = 'diajukan'
     ORDER BY p.realisasi_diajukan`, [penggunaId]).then(baris => {
    for (const b of baris) b.realisasi = bacaJson(b.realisasi_json, {});
    return baris;
  });
}
...
async function jumlahKotakMasuk(penggunaId) {
  ...
  const nRealisasi = await db.nilai(
    `SELECT COUNT(*) AS n FROM pengajuan p
     JOIN persetujuan_realisasi s ON s.pengajuan_id = p.id AND s.status = 'menunggu'
       AND s.urut = (SELECT MIN(urut) FROM persetujuan_realisasi WHERE pengajuan_id = p.id AND status = 'menunggu')
     JOIN persetujuan_realisasi_kandidat kd ON kd.persetujuan_realisasi_id = s.id AND kd.pengguna_id = ?
     WHERE p.status_realisasi = 'diajukan'`, [penggunaId]);
  return Number(n || 0) + Number(nRealisasi || 0);
}
...
module.exports = {
  ambil, daftar, kotakMasuk, kotakMasukRealisasi, jumlahKotakMasuk, ringkasan, rekapKategori,
  ...
  langkahAktifRealisasi, bolehMemutuskanRealisasi,
};
