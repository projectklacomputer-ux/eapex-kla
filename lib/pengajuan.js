...
const PERAN_LIHAT_SEMUA = ['admin', 'accounting', 'ceo', 'regional_manager'];
...
async function daftar(f = {}) {
  ...
  // Pembatasan tampilan menurut peran (gagal-tertutup: bukan pengawas -> hanya
  // dokumen sendiri atau dokumen yang ia menjadi kandidat penyetujunya).
  if (f.pengguna && !PERAN_LIHAT_SEMUA.includes(f.pengguna.peran)) {
    if (f.pengguna.peran === 'area_manager' && f.pengguna.area_id) {
      syarat.push(`(p.pemohon_id = ? OR c.area_id = ? OR EXISTS (
        SELECT 1 FROM persetujuan_kandidat kd WHERE kd.pengajuan_id = p.id AND kd.pengguna_id = ?))`);
      params.push(f.pengguna.id, f.pengguna.area_id, f.pengguna.id);
    } else {
      syarat.push(`(p.pemohon_id = ? OR EXISTS (
        SELECT 1 FROM persetujuan_kandidat kd WHERE kd.pengajuan_id = p.id AND kd.pengguna_id = ?))`);
      params.push(f.pengguna.id, f.pengguna.id);
    }
  }
  const where = syarat.length ? 'WHERE ' + syarat.join(' AND ') : '';
  const batas = Math.min(Number(f.batas || 100), 500);
  const mulai = Math.max(Number(f.mulai || 0), 0);
...
async function ringkasan(pengguna) {
  ...
  const bulanIni = await db.get(
    `SELECT COUNT(*) AS n, COALESCE(SUM(total),0) AS jml FROM pengajuan
     WHERE status = 'disetujui' AND ditutup >= ?`, [sejak]);
  const berjalan = await db.get(
    `SELECT COUNT(*) AS n, COALESCE(SUM(total),0) AS jml FROM pengajuan WHERE status = 'menunggu'`);
  return {
...
async function rekapKategori(pengguna) {
  ...
  return db.all(
    `SELECT k.nama, k.grup, COUNT(*) AS n, COALESCE(SUM(p.total),0) AS jml
     ...
     WHERE p.status <> 'draft' AND p.dibuat >= ?
     GROUP BY k.nama, k.grup ORDER BY jml DESC`, [sejak]);
}
