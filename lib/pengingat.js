...
const { rp, sekarang, selisihHari, selisihHariZona } = require('./util');
...
// Dua ambang, sengaja berbeda. Draft dihapus setelah 2 hari didiamkan, tapi
// peringatannya dikirim SEHARI SEBELUMNYA. Menghapus pekerjaan orang tanpa
// peringatan lebih dulu adalah kehilangan yang tidak bisa ditarik kembali -
// orang yang mulai mengisi Jumat sore dan kembali Senin pagi tidak boleh
// mendapati pekerjaannya lenyap tanpa pernah diberi tahu.
const HARI_DRAFT = () => {
  const n = Number(process.env.DRAFT_HARI_INGAT);
  return Number.isInteger(n) && n >= 1 ? n : 1;
};
const HARI_HAPUS = () => {
  const n = Number(process.env.DRAFT_HARI_HAPUS);
  return Number.isInteger(n) && n >= 1 ? n : 2;
};

// Ambil semua draft yang MUNGKIN sudah lewat, lalu saring tepat menurut hari
// kalender WIB. Penyaringan halusnya tidak dilakukan di SQL karena SQLite dan
// PostgreSQL menghitung zona waktu dengan cara yang berbeda - satu aturan yang
// ditulis dua kali dalam dua dialek pasti berbeda hasilnya suatu saat.
async function draftKandidat(minHari) {
  const longgar = new Date(Date.now() - (minHari * 24 - 24) * 3600 * 1000).toISOString();
  const baris = await db.all(
    `SELECT u.id AS pengguna_id, u.nama AS pengguna_nama,
     ...
     ORDER BY u.nama, p.diperbarui`, [longgar]);
  return baris.filter(b => selisihHariZona(b.diperbarui) >= minHari);
}

// Yang sudah lewat ambang peringatan TAPI belum lewat ambang hapus. Tanpa batas
// atas, dokumen yang hari ini dihapus akan ikut diperingatkan pada putaran yang
// sama - orang menerima dua kabar bertentangan sekaligus.
async function draftTerbengkalai() {
  const semua = await draftKandidat(HARI_DRAFT());
  return semua.filter(b => selisihHariZona(b.diperbarui) < HARI_HAPUS());
}

// Menghapus draft yang sudah lewat tenggat, berikut SELURUH jejaknya - termasuk
// isi lampiran di dalam basis data. Kalau hanya baris `pengajuan` yang dihapus,
// isi berkasnya tetap menghuni Supabase selamanya tanpa ada yang menunjuk
// kepadanya: ruang terpakai, tidak terlihat siapa pun, tidak bisa dibersihkan.
async function hapusDraftTua() {
  const simpanan = require('./simpanan');
  const calon = (await draftKandidat(HARI_HAPUS()))
    .map(b => ({ id: b.pengajuan_id, judul: b.judul, total: b.total,
      pemohon_id: b.pengguna_id, diperbarui: b.diperbarui }));
  if (!calon.length) return { jumlah: 0, penerima: 0 };

  const perOrang = new Map();
  for (const d of calon) {
    const lampiran = await db.all('SELECT nama_simpan FROM lampiran WHERE pengajuan_id = ?', [d.id]);
    for (const l of lampiran) {
      try { await simpanan.hapus(l.nama_simpan); } catch (e) {
        console.warn('[draft] isi lampiran gagal dihapus:', l.nama_simpan, e.message);
      }
    }
    // Urutan penting: anak-anaknya dulu, baru induknya.
    await db.run('DELETE FROM lampiran WHERE pengajuan_id = ?', [d.id]);
    await db.run('DELETE FROM pengajuan_item WHERE pengajuan_id = ?', [d.id]);
    await db.run('DELETE FROM persetujuan_kandidat WHERE pengajuan_id = ?', [d.id]);
    await db.run('DELETE FROM persetujuan WHERE pengajuan_id = ?', [d.id]);
    await db.run('DELETE FROM jejak WHERE pengajuan_id = ?', [d.id]);
    await db.run('DELETE FROM notifikasi WHERE pengajuan_id = ?', [d.id]);
    await db.run('DELETE FROM pengajuan WHERE id = ?', [d.id]);

    if (!perOrang.has(d.pemohon_id)) perOrang.set(d.pemohon_id, []);
    perOrang.get(d.pemohon_id).push(d);
  }

  for (const [penggunaId, dokumen] of perOrang) {
    const n = dokumen.length;
    const daftar = dokumen.slice(0, 10)
      .map(d => `• ${d.judul || '(tanpa perihal)'} (${rp(d.total)})`).join('\n');
    const kabar = {
      judul: n === 1 ? 'Draft Anda dihapus karena didiamkan' : `${n} draft Anda dihapus karena didiamkan`,
      pesan: `${daftar}\n\nDraft yang tidak disentuh ${HARI_HAPUS()} hari akan dihapus beserta `
        + 'lampirannya. Tidak ada nomor dokumen yang hilang - draft memang belum pernah punya nomor. '
        + 'Kalau masih dibutuhkan, silakan buat pengajuan baru.',
      url: '/pengajuan/baru',
      tag: 'draft-dihapus',
    };
  ...
  return { jumlah: calon.length, penerima: perOrang.size };
}
...
function susunPesanDraft(dokumen) {
  ...
  baris.push(`PERHATIAN: draft yang tidak disentuh ${HARI_HAPUS()} hari akan DIHAPUS`);
  baris.push('beserta lampirannya. Selesaikan lalu ajukan hari ini.');
  return baris.join('\n');
}
...
async function jalankan({ paksa = false } = {}) {
  ...
  // Hapus yang sudah lewat tenggat LEBIH DULU, baru ingatkan sisanya. Kalau
  // dibalik, orang menerima peringatan untuk draft yang sudah terhapus.
  const dihapus = await hapusDraftTua();
