...
async function hapusDraftTua() {
  ...
  const batas = new Date(Date.now() - HARI_HAPUS() * 24 * 3600 * 1000).toISOString();

  const calon = await db.all(
    `SELECT p.id, p.judul, p.total, p.pemohon_id, p.diperbarui
       FROM pengajuan p
      WHERE p.status = 'draft' AND p.diperbarui < ?`, [batas]);
