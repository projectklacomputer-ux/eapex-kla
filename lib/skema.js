...
-- Rincian realisasi per baris (Transport, Hotel, Uang Makan, dst) -- struktur
-- sama persis pengajuan_item, dipakai form.bacaItems() yang sama. Dibangun
-- ulang tiap ajukanRealisasi() (delete lalu insert), sama seperti pola tabel
-- persetujuan_realisasi.
CREATE TABLE IF NOT EXISTS pengajuan_realisasi_item (
  id TEXT PRIMARY KEY,
  pengajuan_id TEXT NOT NULL,
  urut INTEGER NOT NULL DEFAULT 0,
  nama TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_realisasi_item_pengajuan ON pengajuan_realisasi_item (pengajuan_id, urut);
...
-- Rantai approval ULANG untuk realisasi uang muka (pertanggungjawaban perjalanan
-- dinas). Struktur SENGAJA disalin persis dari persetujuan/persetujuan_kandidat
-- alih-alih menaruh flag "putaran" di tabel yang sama: rantai asli (uang muka)
-- dan rantai realisasi harus bisa hidup berdampingan tanpa saling mengganggu
-- kotakMasuk, penghitung "X dari Y tahap", maupun cetakan dokumen asli — yang
-- semuanya sudah teruji dan tidak boleh disentuh oleh fitur ini.
-- Dibangun ulang dari nol (DELETE lalu INSERT) tiap kali pemohon mengajukan
-- realisasi, sama seperti persetujuan dibangun ulang tiap dokumen diajukan.
CREATE TABLE IF NOT EXISTS persetujuan_realisasi (
  id TEXT PRIMARY KEY,
  pengajuan_id TEXT NOT NULL,
  urut INTEGER NOT NULL,
  peran TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_persetujuan_realisasi_pengajuan ON persetujuan_realisasi (pengajuan_id, urut);
CREATE INDEX IF NOT EXISTS ix_persetujuan_realisasi_status ON persetujuan_realisasi (status);

CREATE TABLE IF NOT EXISTS persetujuan_realisasi_kandidat (
  id TEXT PRIMARY KEY,
  persetujuan_realisasi_id TEXT NOT NULL,
  pengajuan_id TEXT NOT NULL,
  pengguna_id TEXT NOT NULL
);
CREATE INDEX IF NOT EXISTS ix_kandidat_realisasi_pengguna ON persetujuan_realisasi_kandidat (pengguna_id);
CREATE INDEX IF NOT EXISTS ix_kandidat_realisasi_persetujuan ON persetujuan_realisasi_kandidat (persetujuan_realisasi_id);
...
async function siapkan(opsi = {}) {
  ...
  // Uang muka -> penyelesaian perjalanan dinas. SATU dokumen, SATU nomor dari
  // awal sampai akhir — bukan dua dokumen yang saling merujuk. Semua kolom
  // nullable/berdefault: baris lama otomatis perlu_realisasi=0, tidak masuk
  // alur ini sama sekali, dan tidak ada backfill data yang perlu dijalankan.
  //
  //   perlu_realisasi     dipilih pemohon SEKALI saat mengajukan, tidak
  //                       pernah berubah sesudahnya
  //   status_realisasi    NULL (belum berlaku) | menunggu (sudah disetujui,
  //                       pemohon belum isi) | diajukan (menunggu verifikasi
  //                       Accounting) | revisi | selesai
  //   realisasi_json      nominal, tanggal, keterangan — bentuk sama seperti
  //                       data_json, dibaca lib/formulir.js
  await tambahKolom('pengajuan', 'perlu_realisasi', 'INTEGER NOT NULL DEFAULT 0');
  await tambahKolom('pengajuan', 'status_realisasi', 'TEXT');
  await tambahKolom('pengajuan', 'realisasi_json', 'TEXT');
  await tambahKolom('pengajuan', 'realisasi_diajukan', 'TEXT');
  await tambahKolom('pengajuan', 'realisasi_selesai', 'TEXT');
  await tambahKolom('pengajuan', 'realisasi_verif_id', 'TEXT');
  await tambahKolom('pengajuan', 'realisasi_catatan', 'TEXT');

  // Membedakan lampiran penawaran awal dari bukti/kwitansi realisasi, supaya
  // keduanya bisa ditampilkan terpisah alih-alih tercampur dalam satu daftar.
  // DEFAULT 'penawaran' untuk alasan yang sama seperti lampiran_wajib di atas:
  // baris lama (dan baris baru yang dibuat lewat jalur lampiran biasa) harus
  // otomatis benar tanpa migrasi data terpisah.
  await tambahKolom('lampiran', 'jenis', "TEXT NOT NULL DEFAULT 'penawaran'");
