...
// server.
const RAHASIA_CSRF = () => process.env.SESSION_SECRET
  || 'rahasia-pengembangan-lokal-jangan-dipakai-di-produksi';

function tokenCsrf(req) {
  if (!req.sessionID) return '';
  return crypto.createHmac('sha256', RAHASIA_CSRF())
    .update('csrf:' + req.sessionID)
    .digest('hex')
    .slice(0, 48);
}
...
function tokenCocok(req) {
  ...
  if (samaAman(kiriman, tokenCsrf(req))) return true;
  // Masa peralihan: halaman yang sudah terbuka SEBELUM perubahan ini membawa
  // token lama yang tersimpan di sesi. Menolaknya berarti setiap orang yang
  // sedang mengisi formulir kehilangan isiannya saat penyebaran berjalan.
  // Sesi lama habis sendiri dalam 60 menit dan cabang ini ikut mati.
  return !!(req.session && req.session.csrf) && samaAman(kiriman, req.session.csrf);
}

function csrf(req, res, next) {
  // Sesi tetap harus TERBIT, kalau tidak cookie-nya tidak pernah dikirim dan
  // permintaan berikutnya datang dengan ID sesi baru - tokennya ikut berbeda
  // dan kiriman pertama selalu ditolak. Yang ditulis sekadar penanda tetap;
  // nilainya tidak menentukan token, jadi permintaan bersamaan yang menimpanya
  // tidak merusak apa pun. Inilah bedanya dengan menyimpan tokennya sendiri.
  if (req.session && !req.session.awal) req.session.awal = 1;

  res.locals.csrf = tokenCsrf(req);
