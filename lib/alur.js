...
async function ajukanRealisasi(pengajuanId, pengguna, { items = [], tanggal, keterangan, berkas = [] }, ip) {
  const p = await P.ambil(pengajuanId);
  if (!p) throw new GalatAlur('Pengajuan tidak ditemukan', 404);
  ...
  // Rincian per baris (Transport, Hotel, Uang Makan, dst) -- sama seperti
  // rincian biaya uang muka, bukan satu angka total. Nominal dihitung dari
  // penjumlahan baris, bukan diinput manual, supaya tidak bisa mencong dari isinya.
  if (!items.length) throw new GalatAlur('Rincian realisasi wajib diisi minimal satu baris');
  const nominal = P.hitungTotal('perjalanan', {}, items);
  if (!(nominal > 0)) throw new GalatAlur('Total rincian realisasi wajib lebih dari nol');
  ...
  await db.tx(async ops => {
    ...
    await ops.run(
      ...
      [JSON.stringify({ nominal, tanggal, keterangan: keterangan || '' }), waktu, pengajuanId]);
    await ops.run('DELETE FROM pengajuan_realisasi_item WHERE pengajuan_id = ?', [pengajuanId]);
    for (const it of items) {
      await ops.run(
        `INSERT INTO pengajuan_realisasi_item (id, pengajuan_id, urut, nama, qty, satuan, harga, nominal, keterangan)
         VALUES (?,?,?,?,?,?,?,?,?)`,
